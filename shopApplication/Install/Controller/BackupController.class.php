...
class BackupController extends MyController {
    public $config = '';                                                        // 相关配置
    ...
    function index() {
        ...
        $getTotalTables = count($list);
        $this->assign('counts', $getTotalTables);
    ...
    function fileList() {
        ...
        $this->assign('dbname', $this->dbName);
        $this->assign('counts', $getCounts);
        $this->assign('list', $list);
        $this->display('backup_restore');
    }
     ...
     */
    function backAll() {
        $getOption = I('get.type');
        if (isset ($getOption) && $getOption == 'all')
        {
    ...
    public function doBatch ()
    {
        ...
        if (isset($_POST['de']))
        {
            foreach ($post as $val)
            {
                if (unlink($this->config['path'] . $this->dir_sep . $val . '.sql'))
                {
                    $count++;
                }
            }
    ...
    //删除数据备份
    function deleteBak() {
        $getFile = $_GET['file'] . '.sql';
        if (unlink($this->config['path'] . $this->dir_sep . $getFile)) {
            $this->success('删除备份成功！');
        } else {
            $this->error('删除备份失败！');
        }
